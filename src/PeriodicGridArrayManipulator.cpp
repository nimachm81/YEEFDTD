...
std::array<RealNumber, 3> PeriodicGridArrayManipulator::BringPointInsideUnitCell(const std::array<RealNumber, 3>& r) {
    ...
    RealNumber rem_0 = std::fmod(std::real(r_dot_v0), std::real(v0_len));
    ...
    RealNumber rem_1 = std::fmod(std::real(r_dot_v1), std::real(v1_len));
    ...
    RealNumber rem_2 = std::fmod(std::real(r_dot_v2), std::real(v2_len));
